...
export class Level6 extends Scene {
    ...
    processCommand(command: string) {
        ...
        // CREATE NODE2 ONLY AFTER CONNECTION COMMAND
        this.addingPlatform2(from, direction, to);
    ...
    addingPlatform2(from: number, direction: string, to: number) {
        if (from === 1 && direction === "next" && to === 2) {
            ...
            this.createPlatform(this.spawnx + 350, this.spawny + 200, 2);

            // Create the collectible after node2 exists
            this.createItemOnPlatform(2, "key");

            // Refresh visuals + physics
            this.drawAll();
            this.updatePlatformStates();
        }
    }
    ...
    create() {
        ...
        this.spawnx = 100;
        this.spawny = 150;
        this.player = this.physics.add.sprite(200, 150, "dude");
        ...
        this.createPlatform(this.spawnx + 150, this.spawny + 200, 1);
        // Missing platform 2 is added when player connects node1->next=node2
        this.createPlatform(this.spawnx + 550, this.spawny + 200, 3);
        this.createPlatform(this.spawnx + 750, this.spawny + 200, 4);
        this.createPlatform(this.spawnx + 950, this.spawny + 200, 5);
        this.createPlatform(this.spawnx + 1150, this.spawny + 200, 6);
    ...
    update() {
        ...
        if (this.health <= 0) {
            ...
            this.scene.restart();
            this.scene.start("GameOver", { returnTo: "Level6" });
            this.physics.resume();
        }
    }
}
